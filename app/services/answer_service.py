...
class AnswerService:
    ...
    async def chatProcess(self, conversationId: int, request: UserRequest, userId: UUID) -> ResponseToUserRequest:
        response = await self.chatService.processUserRequest(conversationId,request, userId)        
        return response
